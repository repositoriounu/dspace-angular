...
export class BrowseByDatePageComponent extends BrowseByMetadataPageComponent {
  ...
  updateStartsWithOptions(
    ...
    const firstItemRD$: Observable<RemoteData<Item>> =
      this.browseService.getFirstItemFor(definition, scope, SortDirection.ASC);
    const lastItemRD$: Observable<RemoteData<Item>> =
      this.browseService.getFirstItemFor(definition, scope, SortDirection.DESC);
    this.loading$ = observableCombineLatest([firstItemRD$, lastItemRD$]).pipe(
      map(
        ([firstItemRD, lastItemRD]: [RemoteData<Item>, RemoteData<Item>]) =>
          firstItemRD.isLoading || lastItemRD.isLoading
      )
    );
    this.subs.push(
      observableCombineLatest([firstItemRD$, lastItemRD$]).subscribe(
        ([firstItemRD, lastItemRD]: [RemoteData<Item>, RemoteData<Item>]) => {
          let lowerLimit: number = this.getLimit(
            firstItemRD,
            metadataKeys,
            this.appConfig.browseBy.defaultLowerLimit
          );
          // let upperLimit: number = this.getLimit(
          //   lastItemRD,
          //   metadataKeys,
          //   new Date().getUTCFullYear()
          // );
          let upperLimit = new Date().getUTCFullYear();
          const options: number[] = [];
          const oneYearBreak: number =
            Math.floor(
              (upperLimit - this.appConfig.browseBy.oneYearLimit) / 5
            ) * 5;
          const fiveYearBreak: number =
            Math.floor(
              (upperLimit - this.appConfig.browseBy.fiveYearLimit) / 10
            ) * 10;
  ...
  private getLimit(
    ...
    if (hasValue(itemRD.payload)) {
      ...
      if (isNotEmpty(date) && isValidDate(date)) {
        ...
        return isNaN(dateObj.getUTCFullYear())
          ? limit
